...
model.param.set('h_g', '2[nm]', 'gap height');
...
model.param.set('V_amp', '-1.75[V]', 'voltage amplitude');
model.param.set('t_rise', '20e-6[s]', 'pulse rise time');
model.param.set('k_eff','10','ratio of energy relaxation time balastic/diffusive');
...
%assign material parameter for HfO2/gap
model.param.set('EC_gap', '3e3[S/m]', 'Electrical Conductivity coefficient gap');
...
model.component('mod1').variable('var1').set('sigma_fil','exp(-alpha_fil*log(abs(V_amp)/(1[V]*R*atomic_vibration)))*exp(sqrt(abs(V/(8.617e-5[V/K]*T))))');
...
model.component('mod1').variable('var2').set('sigma_gap','exp(-alpha_gap*log(abs(V_amp)/(1[V]*R*atomic_vibration)))*exp(sqrt(abs(V/(8.617e-5[V/K]*T))))');
...
model.component('mod1').mesh('mesh1').feature('ftri4').feature('size1').set('hmax', '0.5e-9');
