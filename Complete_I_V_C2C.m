...
for ramp_rate_loop = 1 : length(ramp_rate)
    ...
    for cycle_loop = 1 : cycle
        %%%%____Randomizing the energy barrier dependence_____%%%%%%%%
        alpha_fil = -(0.03+(0.07-0.03)*rand);%complete 8
        alpha_gap = 0.03+(0.07-0.03)*rand;
        EC_HfO2x = 2e3+(8e3-2e3)*rand;%complete 9
        EC_gap = 1e3+(5e3-1e3)*rand;
        delta_mu_SET_J = 85e8+(115e8-85e8)*rand;
        delta_mu_RESET_J = 55e8+(75e8-55e8)*rand;
        nucleation_barrier = (2.4+(2.6-2.4)*rand)*1.6e-19;
        %%%%%%%%%%%%%%%%%%%%%%%%%Positive Voltage%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%OFF_MODE%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        model_OFF= RRAM_OFF;
...
save('Complete_I_V_C2C_DATA.mat','I_V','DATA_OFF','DATA_min_SET','DATA_SET','DATA_ON','DATA_ON_neg','DATA_min_RESET','DATA_RESET','DATA_OFF_neg')
